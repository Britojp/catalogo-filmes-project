<template>
  ...
  <div class="d-flex flex-row mb-6">
    <template v-if="popularFilms.length">
      <v-carousel
      ...
      >
        <v-carousel-item v-for="(group, index) in groupedFilms" :key="index">
          <v-row class="d-flex justify-center">
            <v-col v-for="film in group" :key="film.id" cols="10" md="3" sm="3">
              <v-hover v-slot="{ props }">
                <v-card max-width="344" v-bind="props" class="ma-2 pa-2 mx-auto">
                  ...
                  />
                  <v-card-text>
                    <h2 class="text-h6 text-primary">{{ film.title }}</h2>
                    <p>
                      {{ expandedFilms.has(film.id) ? film.overview : truncatedText(film) }}
                    </p>
                    <v-btn
                      v-if="isLongText(film)"
                      variant="text"
                      color="primary"
                      @click="toggleExpand(film)"
                    >
                      {{ expandedFilms.has(film.id) ? "Ver menos" : "Ver mais" }}
                    </v-btn>
                  </v-card-text>
                  <v-card-title>
                    <v-rating
...
export default {
  ...
  methods: {
    ...
    isLongText(film: Film) {
      return film.overview.split(" ").length > 20; 
    },
    truncatedText(film: Film) {
      return this.isLongText(film) ? film.overview.split(" ").slice(0, 20).join(" ") + "..." : film.overview;
    },
    toggleExpand(film: Film) {
      if (this.expandedFilms.has(film.id)) {
        this.expandedFilms.delete(film.id);
      } else {
        this.expandedFilms.add(film.id);
      }
    },
  },
};
